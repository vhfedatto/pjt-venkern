...
import {
  ArrowLeft, Phone, Mail, Building2, CalendarClock, FileText,
  MessageCircle, Plus, Clock, ClipboardList, Download, Send, Loader2, Trash2,
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="h-2" style={{ background: team?.color ?? '#6366f1' }} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.35fr_.65fr] gap-5">
        <div className="space-y-5">
          <section className="rounded-3xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 shadow-sm">
            ...
            <div className="rounded-2xl bg-gray-50 dark:bg-gray-800/60 p-3 mb-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-[160px_1fr_auto] gap-2">
                <select value={interactionType} onChange={e => setInteractionType(e.target.value as InteractionType)}
                ...
                <input value={description} onChange={e => setDescription(e.target.value)} placeholder="Ex: cliente pediu retorno amanhã às 10h"
                  className="px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-700 dark:text-gray-200 outline-none" />
            ...
            {interactions.length === 0 ? (
              ...
              <div className="space-y-3">
                {interactions.map(item => {
                  ...
                  return (
                    <div key={item.id} className="relative pl-6 pb-4 border-l border-indigo-100 dark:border-indigo-900 last:pb-0">
          ...
          <section className="rounded-3xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 shadow-sm">
            ...
            {contactTasks.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 p-8 text-center text-sm text-gray-400">Nenhuma tarefa vinculada.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {contactTasks.map(task => (
                  <div key={task.id} className="rounded-2xl bg-gray-50 dark:bg-gray-800/60 p-4">
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-3xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 shadow-sm">
            ...
            />
            {documents.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 p-6 text-center text-sm text-gray-400">Nenhum documento anexado.</div>
            ) : (
              <div className="space-y-2">
                {documents.map(doc => (
                  <div key={doc.id} className="flex items-center gap-3 rounded-2xl bg-gray-50 dark:bg-gray-800/60 p-3">
