...
export default function Groups() {
  ...
  const accessibleGroups = currentUser.role === 'admin'
    ? groups
    : groups.filter(g => g.type === 'general' || g.memberIds.includes(myContactId));
  ...
  useEffect(() => {
    if (!socket) return;
    const onTyping = (data: { conversationId?: number; groupId?: number; name: string; isTyping: boolean }) => {
      setTypingUsers(prev =>
        data.isTyping
    ...
    return () => { socket.off('group:typing', onTyping); };
  ...
  const getTeamColor = (_g: Group) => '#6366f1';
                ...
                <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 240, opacity: 1 }} exit={{ width: 0, opacity: 0 }}
                  ...
                  <div className="p-3 overflow-y-auto space-y-1">
                    {selectedGroup.memberIds.map(id => {
                      ...
                      if (!c) return null;
                      return (
                        <div key={id} className="flex items-center gap-2.5 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800">
