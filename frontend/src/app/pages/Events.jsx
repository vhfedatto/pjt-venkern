...
export default function Events() {
  ...
  const toggleAudience = id => {
    setForm(p => {
      ...
      return {
        ...p,
        targetAudience: newArr.length === 0 ? ['all'] : newArr
      };
    });
  };
  ...
  return <div className="p-4 lg:p-6">
      ...
      y: 0
        ...
        {currentUser.role === 'admin' && <button onClick={openCreate} className="ml-auto flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-medium shadow-md hover:opacity-90 hover:scale-[1.02] transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <AnimatePresence>
          {filtered.map(ev => <motion.div key={ev.id} layout initial={{
          opacity: 0,
          y: 16
        }} animate={{
          opacity: 1,
          ...
          y: -3
              ...
              <div className="relative h-36 overflow-hidden">
                {ev.bannerUrl ? <img src={ev.bannerUrl} alt={ev.title} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 flex items-center justify-center">
                    <Image className="w-12 h-12 text-indigo-300" />
                  </div>}
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                  <EventStatusBadge status={ev.status} />
                  {currentUser.role === 'admin' && <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className="p-4">
                ...
                {currentUser.role === 'admin' && ev.status !== 'sent' && ev.status !== 'completed' && <button onClick={() => handleSend(ev.id)} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-medium hover:opacity-90 transition-opacity shadow-sm">
                    <Send className="w-3.5 h-3.5" /> Enviar para Grupos
                  </button>}
                {(ev.status === 'sent' || ev.status === 'completed') && <div className="flex items-center justify-center gap-2 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-xs font-medium">
      ...
      {/* Event Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editEvent ? 'Editar Evento' : 'Novo Evento'} maxWidth="max-w-xl" footer={<div className="flex gap-3">
        ...
        <div className="space-y-4 pb-2">
          ...
          }))} placeholder="Ex: Kickoff Q2" className={inputCls} />
          </div>
          <div>
          ...
          }))} rows={3} className={`${inputCls} resize-none`} placeholder="Descrição do evento..." />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Data *</label>
              <input type="date" min={today} value={form.date} onChange={e => setForm(p => ({
              ...p,
              date: e.target.value
            }))} className={inputCls} />
            </div>
            <div>
              ...
              <input type="time" value={form.time} onChange={e => setForm(p => ({
              ...p,
              time: e.target.value
            }))} className={inputCls} />
            </div>
          ...
          <div>
            ...
            <input type="text" value={form.location} onChange={e => setForm(p => ({
            ...p,
            location: e.target.value
          }))} placeholder="Ex: Auditório Principal" className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>URL do Banner</label>
            <input type="url" value={form.bannerUrl} onChange={e => setForm(p => ({
            ...p,
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              id: 'all',
              name: 'Todos'
            }, ...teams].map(t => <button key={t.id} type="button" onClick={() => toggleAudience(t.id)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${form.targetAudience.includes(t.id) ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                  {t.name}
                </button>)}
            </div>
          </div>
          <div>
            ...
            <select value={form.status} onChange={e => setForm(p => ({
            ...p,
      ...
      <ConfirmModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={() => deleteId && handleDelete(deleteId)} title="Excluir evento?" message="Esta ação não pode ser desfeita." confirmLabel="Excluir" danger />
    </div>;
}