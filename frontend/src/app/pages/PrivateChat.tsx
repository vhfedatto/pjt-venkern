...
export default function PrivateChat() {
  ...
  useEffect(() => {
    if (!socket) return;
    const onTyping = (data: { conversationId: number; name: string; isTyping: boolean }) => {
  ...
  const formatDate = (ts: string) => new Date(ts).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' });
  ...
  const otherContacts = contacts.filter(
    c => c.id !== myContactId &&
    !myChats.some(ch => ch.participantIds.includes(c.id) && ch.participantIds.includes(myContactId))
  );
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="w-72 flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          <div className="mt-3 px-2">
            ...
            {otherContacts.map(c => (
              <button key={c.id} onClick={() => handleOpenChat(c.id)}
      ...
      {selectedChat && otherContact ? (
        <div className="flex-1 flex flex-col min-w-0">
                  ...
                  <div key={msg.id}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className={`max-w-xs lg:max-w-md flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                        {msg.blocked ? (
                          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/40">
                            <ShieldAlert className="w-4 h-4 text-red-500 flex-shrink-0" />
                            <span className="text-sm text-red-600 dark:text-red-400">{msg.content}</span>
                          </div>
                        ) : (
                          <div className={`px-4 py-2.5 rounded-2xl text-sm shadow-sm ${isMine ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-br-sm' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-bl-sm border border-gray-100 dark:border-gray-700'}`}>
                            {msg.content}
                          </div>
                        )}
          ...
          <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 flex-shrink-0">
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleImageSend} title="Enviar imagem"
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 bg-gray-50 dark:bg-gray-950">
