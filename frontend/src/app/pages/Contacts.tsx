...
const emptyForm = { name: '', email: '', phone: '', role: 'professional' as const, teamId: '', position: '', notes: '' };
...
export default function Contacts() {
  ...
  const [usernameResults, setUsernameResults] = useState<{ id: number; name: string; email: string; username: string | null }[]>([]);
  ...
  const handleUsernameSearch = (q: string) => {
    ...
    const clean = q.trim();
    if (clean.length < 2) { setUsernameResults([]); setShowUsernameDrop(false); return; }
    usernameTimer.current = setTimeout(async () => {
      setUsernameSearching(true);
      try {
        const results = await usersApi.search(clean) as { id: number; name: string; email: string; username: string | null }[];
  ...
  const selectUsernameUser = (u: { name: string; email: string; username: string | null }) => {
  ...
  const openCreate = () => { setEditContact(null); setForm(emptyForm); setUsernameQuery(''); setUsernameResults([]); setShowUsernameDrop(false); setModalOpen(true); };
  const openEdit = (c: Contact) => { setEditContact(c); setForm({ name: c.name, email: c.email, phone: c.phone, role: c.role, teamId: c.teamId, position: c.position, notes: c.notes }); setModalOpen(true); };

  const handleSave = async () => {
    if (!form.name.trim() || !form.email.trim() || !form.phone.trim()) { toast.error('Preencha os campos obrigatórios!'); return; }
    setSaving(true);
    try {
      if (editContact) {
  ...
  return (
    <div className="p-4 lg:p-6">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative flex-1 min-w-52">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Buscar contato..." className={`${inputCls} pl-10 pr-8`} />
          ...
          <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <AnimatePresence>
              {filtered.map(c => {
                const team = teams.find(t => t.id === c.teamId);
                return (
                  <motion.div key={c.id} layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}
                    whileHover={{ y: -3 }}
                    ...
                    {/* Color strip */}
                    <div className="h-1.5" style={{ background: team?.color ?? '#6366f1' }} />
                    <div className="p-5">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}
        ...
        subtitle={editContact ? 'Atualize as informações' : 'Preencha os dados do novo contato'}
        footer={
        ...
        <div className="space-y-4 pb-2">
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="@username ou nome para pré-preencher..."
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Ex: João Silva" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Telefone *</label>
              <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} placeholder="(11) 99999-9999" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Email *</label>
              <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="joao@empresa.com" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Cargo</label>
              <input type="text" value={form.position} onChange={e => setForm(p => ({ ...p, position: e.target.value }))} placeholder="Ex: Desenvolvedor" className={inputCls} />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value as any }))} className={inputCls}>
            ...
            <div className="col-span-2">
              ...
              <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} rows={2} placeholder="Observações sobre o contato..." className={`${inputCls} resize-none`} />
            </div>
          </div>
        </div>
      </Modal>
      ...
      <ConfirmModal isOpen={!!deleteId} onClose={() => setDeleteId(null)}
        ...
        message={`Tem certeza que deseja excluir "${contacts.find(c => c.id === deleteId)?.name}"? Esta ação não pode ser desfeita.`}
