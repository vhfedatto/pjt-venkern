...
export default function Dashboard() {
  ...
  const tasksByColumn = [
    ...
    { name: 'Revisão', value: tasks.filter(t => t.column === 'review').length, color: '#f59e0b' },
    { name: 'Concluído', value: tasks.filter(t => t.column === 'done').length, color: '#10b981' },
  ...
  const statCards = [
    ...
    { label: 'Tarefas Concluídas',value: stats.doneTasks,     icon: CheckCircle2, color: 'from-green-500 to-green-700',  border: 'border-green-100 dark:border-green-900',  path: '/kanban' },
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Aqui está o resumo de hoje — {new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
      </motion.div>

      {/* Stats grid */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {statCards.map(({ label, value, icon: Icon, color, border, path }, i) => (
          <motion.div key={label} whileHover={{ scale: 1.02, y: -2 }} transition={{ duration: 0.15 }}
        ...
        <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-2">
              {contactsByTeam.map(({ name, value, color }) => (
          ...
          {lateTasks.length === 0 ? (
            ...
            <div className="space-y-2">
              {lateTasks.map(t => {
                ...
                return (
                  <div key={t.id} className="flex items-start gap-2.5 p-2.5 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/40">
          ...
          {upcomingEvents.length === 0 ? (
            ...
            <div className="space-y-3">
              {upcomingEvents.map(ev => (
                <div key={ev.id} className="flex gap-3 p-2.5 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/40">
                  <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/40 flex flex-col items-center justify-center flex-shrink-0">
                    <span className="text-blue-700 dark:text-blue-300 text-xs font-bold leading-none">
                      {new Date(ev.date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit' })}
                    </span>
