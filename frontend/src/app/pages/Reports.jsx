...
import { TrendingUp, Users, ListChecks, Shield, Loader2, RefreshCw, MessageSquare, Calendar, TriangleAlert, Star } from '../components/ui/Icons';
...
function ChartCard({
  ...
  return <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5">
...
function EmptyChart({
  ...
  return <div className="flex items-center justify-center h-[180px] text-sm text-gray-400">
      {message}
    </div>;
}
function PieLegend({
...
export default function Reports() {
  ...
  const {
    ...
    error,
    refresh
  } = useReports(currentProject?.id);
  if (!currentProject) {
    return <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] gap-3">
  ...
  if (error) {
    return <div className="p-6 flex flex-col items-center justify-center gap-3 min-h-[40vh]">
        <p className="text-red-500 text-sm">{error}</p>
        <button onClick={refresh} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-500 text-white text-sm hover:bg-indigo-600">
  ...
  const summaryStats = summary ? [{
    ...
    label: 'Alertas',
    value: summary.totalModerationAlerts,
    icon: Shield,
  ...
  return <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={refresh} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 hover:text-indigo-500 transition-colors" title="Atualizar">
          <RefreshCw className="w-4 h-4" />
        </button>
      </div>

      {/* Summary cards */}
      {summaryStats.length > 0 && <motion.div initial={{
      opacity: 0,
      y: -12
    }} animate={{
      opacity: 1,
      ...
      delay: 0.08
        ...
        <ChartCard title="Atividade ao Longo do Tempo">
          {activityChart.length === 0 ? <EmptyChart message="Nenhuma atividade registrada ainda." /> : <ResponsiveContainer width="100%" height={220}>
              ...
              fontSize: 11
                ...
                <Line type="monotone" dataKey="Contatos" stroke="#6366f1" strokeWidth={2} dot={{
              r: 3
            }} />
                <Line type="monotone" dataKey="Tarefas" stroke="#3b82f6" strokeWidth={2} dot={{
              r: 3
            }} />
                <Line type="monotone" dataKey="Concluídas" stroke="#10b981" strokeWidth={2} dot={{
              r: 3
            }} />
                <Line type="monotone" dataKey="Mensagens" stroke="#f59e0b" strokeWidth={2} dot={{
              r: 3
            }} />
              </LineChart>
            </ResponsiveContainer>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        delay: 0.1
          ...
          <ChartCard title="Tarefas por Status">
            {tasksByStatus.every(r => r.count === 0) ? <EmptyChart message="Nenhuma tarefa criada." /> : <ResponsiveContainer width="100%" height={180}>
        ...
        delay: 0.1
          ...
          <ChartCard title="Tarefas por Prioridade">
            {tasksByPriority.every(r => r.count === 0) ? <EmptyChart message="Nenhuma tarefa criada." /> : <ResponsiveContainer width="100%" height={180}>
        ...
        delay: 0.15
          ...
          <ChartCard title="Contatos por Equipe">
            {contactsByTeam.length === 0 ? <EmptyChart message="Nenhuma equipe encontrada." /> : <div className="flex items-center gap-4">
        ...
        delay: 0.15
          ...
          <ChartCard title="Contatos por Origem">
            {contactsByOriginPie.length === 0 ? <EmptyChart message="Nenhuma origem cadastrada." /> : <div className="flex items-center gap-4">
                <ResponsiveContainer width="55%" height={180}>
                  <PieChart>
        ...
        {/* Top assignees */}
        {tasks && tasks.byAssignee.length > 0 && <motion.div initial={{
        opacity: 0,
        y: 16
      }} animate={{
        opacity: 1,
        ...
        delay: 0.2
            ...
            <ChartCard title="Tarefas por Responsável (Top 10)">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={tasks.byAssignee.map(r => ({
              name: r.name.split(' ')[0],
              count: r.count
