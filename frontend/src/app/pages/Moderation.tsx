...
export default function Moderation() {
  ...
  const formatTime = (ts: string) => new Date(ts).toLocaleString('pt-BR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' });
        ...
        {FILTERS.map(f => (
          <button key={f.value} onClick={() => setFilter(f.value)}
            ...
            {f.value === 'pending' && stats.pending > 0 && (
              <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-red-100 text-red-600 text-xs font-bold">{stats.pending}</span>
            )}
          </button>
        ))}
      ...
      <div className="space-y-3">
        <AnimatePresence>
          {filtered.length === 0 ? (
            ...
            return (
              <motion.div key={alert.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -16 }}
                className={`bg-white dark:bg-gray-900 rounded-2xl border shadow-sm p-4 ${
                  alert.status === 'pending' ? 'border-red-200 dark:border-red-900/60' : 'border-gray-100 dark:border-gray-800'
                }`}>
                <div className="flex items-start gap-4">
                  {/* Icon */}
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    alert.status === 'pending' ? 'bg-red-100 dark:bg-red-900/30' : alert.status === 'resolved' ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-gray-100 dark:bg-gray-800'
                  }`}>
                    <ShieldAlert className={`w-5 h-5 ${
                      alert.status === 'pending' ? 'text-red-500' : alert.status === 'resolved' ? 'text-emerald-500' : 'text-gray-400'
                    }`} />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      ...
                      <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${
                        alert.status === 'pending' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
