...
export default function PrivateChat() {
  ...
  const handleImageSend = async () => {
    ...
    try {
      await sendMessage(selectedChatId, myContactId, '📷 [Imagem anexada]', 'image');
  ...
  const otherContacts = contacts.filter(c => c.id !== myContactId && !myChats.some(ch => ch.participantIds.includes(c.id) && ch.participantIds.includes(myContactId)));
  return <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="w-72 flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col">
        <div className="p-4 border-b border-gray-100 dark:border-gray-800">
          <h2 className="text-sm font-bold text-gray-900 dark:text-white mb-3">Mensagens</h2>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar conversa..." className="w-full pl-9 pr-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-xs text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-400 placeholder:text-gray-400" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-2">
          ...
          return <button key={chat.id} onClick={() => setSelectedChatId(chat.id)} className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all mb-1 ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}>
                <Avatar name={other.name} size="md" status={other.status} className="flex-shrink-0" />
          ...
          <div className="mt-3 px-2">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Outros Contatos</p>
            {otherContacts.map(c => <button key={c.id} onClick={() => handleOpenChat(c.id)} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 text-left transition-colors mb-1">
      ...
      {/* Chat area */}
      {selectedChat && otherContact ? <div className="flex-1 flex flex-col min-w-0">
          {/* Chat header */}
          <div className="flex items-center gap-3 px-5 py-3.5 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
            <Avatar name={otherContact.name} size="md" status={otherContact.status} />
            <div className="flex-1">
              <p className="text-sm font-bold text-gray-900 dark:text-white">{otherContact.name}</p>
              <p className="text-xs text-gray-400">{otherContact.position}</p>
            </div>
            <StatusBadge status={otherContact.status} />
          </div>
          ...
          return <div key={msg.id}>
              ...
              y: 0
                      ...
                      {!isMine && <Avatar name={otherContact.name} size="xs" className="flex-shrink-0 mb-1" />}
          ...
          }} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder={`Mensagem para ${otherContact.name.split(' ')[0]}...`} className="flex-1 px-4 py-2.5 rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-400 transition-all placeholder:text-gray-400" />
              <button onClick={handleSend} disabled={!message.trim() || sending} className="p-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed shadow-sm flex-shrink-0">
                {sending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
              </button>
            </div>
          </div>
        </div> : <div className="flex-1 flex flex-col items-center justify-center gap-4 bg-gray-50 dark:bg-gray-950">
          <div className="w-20 h-20 rounded-3xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center">
