...
function withQuery(path, query) {
  ...
  if (query instanceof URLSearchParams) {
    const qs2 = query.toString();
    return qs2 ? `${path}?${qs2}` : path;
  }
...
const contactsApi = {
  ...
  favorite: (id, value) => api.patch(`/contacts/${id}/favorite`, value === void 0 ? void 0 : { is_favorite: value })
};
const teamsApi = {
...
const tasksApi = {
  ...
  moveStatus: (id, status) => api.patch(`/tasks/${id}/status`, { status }),
...
const chatsApi = {
  list: (participantIdOrProjectId, projectId) => {
    ...
    return api.get(withQuery("/chats", participantIdOrProjectId ? { project_id: participantIdOrProjectId } : void 0));
  },
...
const dashboardApi = {
  summary: (projectId) => api.get(withQuery("/dashboard/summary", { project_id: projectId }))
};
...
const projectsApi = {
  ...
  changeMemberRole: (projectId, memberId, role) => api.patch(`/projects/${projectId}/members/${memberId}/role`, { role }),
