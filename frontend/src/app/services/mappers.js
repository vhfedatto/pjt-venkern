...
export function mapApiContactToUi(contact) {
  return {
...
export function mapUiContactToApi(contact) {
  return {
...
export function mapApiTeamToUi(team) {
  const {
    bg,
    text
  } = getTeamColors(team.color ?? '#6366f1');
  return {
    id: String(team.id),
    name: team.name,
...
export function mapUiTaskToApi(task) {
  return {
    ...
    ...(task.assigneeId !== undefined && {
      assignee_id: task.assigneeId ? Number(task.assigneeId) : null
    }),
    ...(task.teamId !== undefined && {
      team_id: task.teamId ? Number(task.teamId) : null
    }),
    ...(task.tags !== undefined && {
      tags: task.tags
    }),
...
export function mapUiEventToApi(event) {
  return {
    title: event.title,
    description: event.description,
    date: event.date,
...
export function mapApiMessageToUi(msg) {
  return {
    id: String(msg.id),
...
export function mapApiGroupToUi(group) {
  return {
    id: String(group.id),
    name: group.name,
...
export function mapApiChatToUi(chat) {
...
export function mapApiInteractionToUi(interaction) {
  return {
...
export function mapApiDocumentToUi(doc) {
  return {
    id: String(doc.id),
...
export function mapApiModerationAlertToUi(alert) {
  return {
    id: String(alert.id),
