...
import { LayoutDashboard, Users, Trello, MessageSquare, MessageCircle, CalendarDays, Building2, Shield, BarChart3, Settings, X, LogOut, ChevronRight, Boxes, ChevronDown, UserPlus, Mail } from '../ui/Icons';
...
export function Sidebar() {
  ...
  const content = <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 py-5 flex-shrink-0">
        ...
        {/* Mobile close */}
        <button className="lg:hidden ml-auto p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400" onClick={() => setSidebarOpen(false)}>
          <X className="w-4 h-4" />
        </button>
      </div>

      {/* Project selector */}
      {projects.length > 0 && <div className="px-4 mb-3 relative">
          <button onClick={() => setProjectDropdownOpen(v => !v)} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors text-left">
          ...
          {projectDropdownOpen && <div className="absolute left-4 right-4 top-full mt-1 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 z-50 overflow-hidden">
              {projects.map(p => <button key={p.id} onClick={() => {
          setCurrentProject(p);
      ...
      <nav className="flex-1 overflow-y-auto px-3 space-y-0.5">
        ...
        path,
        icon: Icon
      }) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setSidebarOpen(false)} className={({
        isActive
      }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${isActive ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md shadow-indigo-200 dark:shadow-none' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'}`}>
            {({
          isActive
        }) => <>
        ...
        {isAdmin && <>
          ...
          path,
          icon: Icon
        }) => <NavLink key={path} to={path} onClick={() => setSidebarOpen(false)} className={({
          isActive
        }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${isActive ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md shadow-indigo-200 dark:shadow-none' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'}`}>
                {({
            isActive
                    ...
                    <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-indigo-500'}`} />
                    <span className="flex-1">{label}</span>
                    {label === 'Moderação' && <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center ${isActive ? 'bg-white/20 text-white' : 'bg-red-500 text-white'} hidden`}>
                        !
                      </span>}
                    {isActive && <ChevronRight className="w-3.5 h-3.5 text-white/70" />}
                  </>}
        ...
        {currentProject && <>
            ...
            {isAdmin && <NavLink to="/membros" onClick={() => setSidebarOpen(false)} className={({
          isActive
        }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${isActive ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md shadow-indigo-200 dark:shadow-none' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'}`}>
                {({
            isActive
            ...
            <NavLink to="/convites" onClick={() => setSidebarOpen(false)} className={({
          isActive
        }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${isActive ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md shadow-indigo-200 dark:shadow-none' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'}`}>
              {({
            isActive
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 mx-1 my-3" />
        <NavLink to="/configuracoes" onClick={() => setSidebarOpen(false)} className={({
        isActive
      }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${isActive ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md shadow-indigo-200 dark:shadow-none' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'}`}>
          {({
          isActive
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-gray-50 dark:bg-gray-800 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={() => {
        ...
        setSidebarOpen(false);
          ...
          e.stopPropagation();
          logout();
        }} className="p-1 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-400 transition-colors" title="Sair">
