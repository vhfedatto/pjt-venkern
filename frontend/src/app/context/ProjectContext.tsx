...
import { type AuthProject, useAuth } from './AuthContext';

const STORAGE_KEY = 'venkern_project_id';

interface ProjectContextType {
  currentProject: AuthProject | null;
  setCurrentProject: (project: AuthProject) => void;
}
...
export function ProjectProvider({ children }: { children: ReactNode }) {
  ...
  const [currentProject, setCurrentProjectState] = useState<AuthProject | null>(() => {
    const storedId = localStorage.getItem(STORAGE_KEY);
    if (!storedId) return null;
    return { id: Number(storedId) } as AuthProject;
  });
  ...
  const setCurrentProject = useCallback((project: AuthProject) => {
